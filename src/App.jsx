...
import { 
  ...
  PlayCircle, GraduationCap, ChevronDown
} from 'lucide-react';
...
} catch (e) { console.warn("Running in local/offline mode."); }

const ARCHETYPES = {
  SOLID: { id: 'SOLID', name: 'Solid Foundation', color: '#059669', icon: <Trophy className="w-8 h-8 md:w-12 md:h-12" />, status: "Legacy Ready", description: "Equilibrium across all three pillars. You drive results while maintaining trust.", prescription: "Ready for multi-unit leadership. Focus on scaling the BFP framework through mentorship.", striveFor: "Legacy Scaling" },
  ...
  BURNOUT: { id: 'BURNOUT', name: 'Burnout Driver', color: '#f97316', icon: <Zap className="w-8 h-8 md:w-12 md:h-12" />, status: "Human Debt High", description: "KPIs met through brute force. High velocity but dangerously low Bedrock.", prescription: "Recover Purpose. Shift from 'Command & Control' to 'Clarity & Care'.", striveFor: "Purpose Recovery" },
  ...
  ACCIDENTAL: { id: 'ACCIDENTAL', name: 'The Accidental Leader', color: '#dc2626', icon: <AlertTriangle className="w-8 h-8 md:w-12 md:h-12" />, status: "Critical Risk", description: "Technical expert leading on instinct. Survival mode across all pillars.", prescription: "BFP Foundations. Enrollment in leadership basic training is mandatory.", striveFor: "Core BFP Foundations" }
};
...
const CURRICULUM = {
  B: {
    ...
    color: "#002147",
    modules: [
      { name: "Module 1: The Safety Gap", desc: "Auditing psychological safety and building the core trust required for high-stakes retail environments." },
      { name: "Module 2: Truth-Default Culture", desc: "Implementing radical transparency and high-integrity communication systems that eliminate floor politics." }
    ]
  },
  F: {
    ...
    color: "#C5A059",
    modules: [
      { name: "Module 3: Lean Retail Execution", desc: "Identifying and removing the operational 'red tape' that kills your team's speed and execution." },
      { name: "Module 4: Agile Decision Loops", desc: "Training your staff to solve store-level friction points in 60 seconds without needing your input." }
    ]
  },
  P: {
    ...
    color: "#64748b",
    modules: [
      { name: "Module 5: The 'Why' Hierarchy", desc: "Connecting repetitive retail tasks to deep personal and community purpose to drive long-term retention." },
      { name: "Module 6: Ownership Mindset", desc: "Shifting your team from 'Clock-In' employees to 'Legacy-Builders' who own the store's results as their own." }
    ]
  }
};
...
const QUESTIONS = [
  ...
  { id: 3, pillar: 'B', text: "Trust within the team is strong enough to avoid micromanagement." },
...
export default function App() {
  ...
  const results = useMemo(() => {
    ...
    const scoresArray = [{id: 'B', val: b}, {id: 'F', val: f}, {id: 'P', val: p}];
    const lowest = scoresArray.reduce((prev, curr) => (prev.val < curr.val) ? prev : curr);

    return { ...arch, scores: { b, f, p }, lowestPillar: lowest.id };
  }, [answers]);

  const saveResults = async () => {
    if (!db || !user || !results) return alert("Offline Mode: Data cannot be synced.");
    setIsSaving(true);
    try {
      ...
      alert("Results Synced to The Campus.");
  ...
  if (view === 'welcome') return (
    <div className="min-h-screen bg-[#FAF9F6] flex flex-col items-center justify-center p-4 md:p-6 text-[#002147]">
      <div className="max-w-xl w-full bg-white p-8 md:p-16 rounded-[32px] md:rounded-[40px] shadow-2xl border-t-[8px] md:border-t-[12px] border-[#002147] text-center">
        <p className="uppercase tracking-widest text-[#C5A059] font-black text-[10px] md:text-xs mb-4">Worthy Retail X-Ray</p>
        <h1 className="text-4xl md:text-6xl font-serif font-black mb-8 leading-tight">Leadership Diagnostic</h1>
        <input type="text" value={userName} onChange={(e)=>setUserName(e.target.value)} placeholder="Enter your name..." className="w-full p-4 md:p-5 border-2 rounded-2xl text-center mb-6 md:mb-8 font-bold outline-none focus:border-[#C5A059]" />
    ...
    <div className="min-h-screen bg-[#cbd5e1] p-4 md:py-12 md:px-6 overflow-x-hidden">
        ...
        <div className="max-w-[1280px] mx-auto space-y-6 md:space-y-12">
            ...
            <div className="report-slide bg-[#FAF9F6] p-8 md:p-24 border-t-[10px] md:border-t-[14px] border-[#002147] shadow-2xl min-h-auto md:min-h-[720px] rounded-[24px] md:rounded-none">
                <h2 className="text-2xl md:text-4xl font-serif font-bold text-[#002147] mb-6 md:mb-12 border-b pb-4 md:pb-8 flex justify-between items-center">
                    Analysis
                    <span className="text-[10px] font-black uppercase tracking-widest text-slate-300">BFP Framework</span>
                </h2>
            ...
            <div className="report-slide bg-[#FAF9F6] p-8 md:p-24 border-t-[10px] md:border-t-[14px] border-[#C5A059] shadow-2xl min-h-auto md:min-h-[720px] rounded-[24px] md:rounded-none">
                <div className="flex items-center gap-4 mb-10 border-b pb-8">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 font-black uppercase text-[10px] tracking-widest mt-1">Custom Development Pathway</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                  <div className="lg:col-span-5 bg-white p-8 md:p-12 rounded-[32px] md:rounded-[48px] border shadow-xl text-center flex flex-col items-center justify-center">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest text-slate-400 mb-2">Priority Pillar</p>
                    <h3 className="text-2xl md:text-3xl font-serif font-bold text-[#002147] mb-8">{CURRICULUM[results.lowestPillar].title.split(':')[0]}</h3>
                    <p className="text-slate-500 leading-relaxed text-sm md:text-base">Your diagnostic indicates a structural fracture within this pillar. We suggest starting your masterclass journey here to stabilize the foundation.</p>
                  </div>

                  <div className="lg:col-span-7 flex flex-col gap-6">
                    <p className="font-bold text-[#002147] uppercase text-xs tracking-widest opacity-50 ml-1">Targeted Learning Modules:</p>
                    {CURRICULUM[results.lowestPillar].modules.map((mod, idx) => (
                      <div key={idx} className="bg-white p-6 md:p-8 rounded-3xl border border-slate-100 hover:border-[#C5A059] transition-all flex items-start gap-5 group shadow-sm">
                    ...
                    <div className="mt-4 flex flex-col md:flex-row items-center gap-4">
                      <button className="w-full md:w-auto bg-[#002147] text-white px-10 py-5 rounded-full font-bold shadow-2xl flex items-center justify-center gap-3 active:scale-95 hover:bg-[#C5A059] transition-all">
                        Enroll in Masterclass <ChevronRight size={18} />
                      </button>
                    </div>
                  </div>
                </div>
        ...
        <style>{`
            ...
            @media print { 
                ...
                .report-slide { 
                    width: 1280px !important; height: 720px !important; border: none !important; box-shadow: none !important; border-radius: 0 !important; page-break-after: always !important; 
                    display: flex !important; flex-direction: column !important; margin: 0 !important; padding: 100px !important;
                } 
        ...
        <div className="max-w-7xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-2xl md:text-3xl font-serif font-bold tracking-tight">Campus Admin</h1>
                </div>
                <button onClick={()=>setView('welcome')} className="font-bold text-[#C5A059] flex items-center gap-2 bg-white px-4 py-2 rounded-lg shadow-sm border"><ArrowLeft size={16}/> Back</button>
            </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left">
                      <thead className="bg-slate-50 border-b">
                          <tr>
                            <th className="p-4 md:p-8 uppercase text-[10px] font-black text-slate-400 tracking-widest">Leader</th>
                            <th className="p-4 md:p-8 uppercase text-[10px] font-black text-slate-400 tracking-widest">Archetype</th>
                            <th className="p-4 md:p-8 uppercase text-[10px] font-black text-slate-400 tracking-widest">B / F / P %</th>
                            <th className="hidden md:table-cell p-8 uppercase text-[10px] font-black text-slate-400 tracking-widest">Date</th>
                          </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-50">
                          {assessments.length === 0 ? (
                              <tr><td colSpan="4" className="p-10 md:p-20 text-center text-slate-400 italic font-serif">Waiting for cloud syncs...</td></tr>
                          ) : assessments.map(e => (
                              <tr key={e.id} className="hover:bg-slate-50/50 transition-colors">
                                  <td className="p-4 md:p-8 font-bold text-sm md:text-lg">{e.userName}</td>
                                  <td className="p-4 md:p-8">
                                    <span className="px-2 py-1 rounded text-[10px] font-black uppercase border" style={{color: ARCHETYPES[e.archetype]?.color, borderColor: ARCHETYPES[e.archetype]?.color}}>{e.archetype}</span>
                                  </td>
