...
const ARCHETYPES = {
  ...
  ACCIDENTAL: { id: 'ACCIDENTAL', name: 'The Accidental Leader', color: '#dc2626', icon: <AlertTriangle className="w-10 h-10 md:w-12 md:h-12" />, status: "Critical Risk", description: "Technical expert leading on instinct. Survival mode across all pillars.", prescription: "BFP Foundations. Enrollment in leadership basic training is mandatory.", striveFor: "Core BFP Foundations" }
};

const CURRICULUM = {
  B: { title: "Bedrock: The Foundation", color: "#002147", modules: [{ name: "Module 1: The Safety Gap", desc: "Auditing psychological safety and trust on the floor." }, { name: "Module 2: Truth-Default Culture", desc: "Implementing transparency and communication systems." }] },
  F: { title: "Fuel: The Velocity", color: "#C5A059", modules: [{ name: "Module 3: Lean Retail Execution", desc: "Removing operational 'red tape' that kills store speed." }, { name: "Module 4: Agile Decision Loops", desc: "Training staff to solve problems in 60 seconds." }] },
  P: { title: "Purpose: The Legacy", color: "#64748b", modules: [{ name: "Module 5: The 'Why' Hierarchy", desc: "Connecting floor tasks to deep personal purpose." }, { name: "Module 6: Ownership Mindset", desc: "Building owner-operators who care about your store's success." }] }
};
...
export default function App() {
  ...
  const saveResults = async () => {
    if (!db || !user) return alert("System Offline: Check Vercel Environment Variables.");
    setIsSaving(true);
    try {
    ...
    <div className="min-h-screen bg-[#FAF9F6] flex flex-col items-center justify-center p-4 md:p-6 text-[#002147]">
      <div className="max-w-xl w-full bg-white p-8 md:p-16 rounded-[40px] shadow-2xl border-t-[12px] border-[#002147] text-center">
        ...
        <div className="space-y-4 mb-8">
            <div className="text-left"><label className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-1 block">Your Identity</label>
            ...
            <div className="text-left relative"><label className="text-[10px] font-black uppercase text-slate-400 ml-2 mb-1 block">Team / Store Code</label>
  ...
  if (view === 'quiz') return (
    <div className="min-h-screen bg-[#FAF9F6] flex flex-col items-center justify-center p-4">
      <div className="max-w-2xl w-full bg-white p-6 md:p-14 rounded-[32px] md:rounded-[40px] shadow-2xl border relative overflow-hidden text-center md:text-left">
  ...
  if (view === 'results' && results) return (
    <div className="min-h-screen bg-[#cbd5e1] p-4 md:py-12 md:px-6 overflow-x-hidden">
        ...
        <div className="max-w-[1280px] mx-auto space-y-6 md:space-y-12">
            <div className="report-slide bg-[#FAF9F6] p-8 md:p-24 border-t-[14px] border-[#002147] shadow-2xl flex flex-col justify-center min-h-[400px] md:min-h-[720px]">
            ...
            <div className="report-slide bg-[#FAF9F6] p-8 md:p-24 border-t-[14px] border-[#002147] shadow-2xl min-h-auto md:min-h-[720px]">
                <h2 className="text-2xl md:text-4xl font-serif font-bold text-[#002147] mb-6 md:mb-12 border-b pb-8 flex justify-between items-center">Analysis <span className="text-[10px] font-black uppercase text-slate-300">BFP Framework</span></h2>
            ...
            <div className="report-slide bg-[#FAF9F6] p-8 md:p-24 border-t-[14px] border-[#C5A059] shadow-2xl min-h-auto md:min-h-[720px]">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                  <div className="lg:col-span-5 bg-white p-8 md:p-12 rounded-[48px] border shadow-xl text-center flex flex-col items-center justify-center">
                    ...
                    <p className="text-slate-500 text-sm md:text-base">Priority Focus: Stabilize this pillar to drive performance.</p>
                  </div>
        ...
        <div className="max-w-7xl mx-auto">
            ...
            {!teamStats ? (
                <div className="bg-white p-20 rounded-[40px] border shadow-sm text-center">
                    <Users size={48} className="mx-auto mb-6 text-slate-200" /><h2 className="text-2xl font-serif font-bold mb-2">No team data found</h2><p className="text-slate-400 mb-8">Ensure your team members use the code <span className="font-black text-[#002147]">{teamCode.toUpperCase()}</span></p><button onClick={()=>setView('welcome')} className="text-[#C5A059] font-black uppercase text-xs tracking-widest underline">Return to Start</button>
                </div>
            ) : (
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                    <div className="lg:col-span-4 space-y-6">
                        <div className="bg-[#002147] text-white p-10 rounded-[40px] shadow-xl"><Users className="text-[#C5A059] mb-4" size={32} /><p className="text-[10px] font-black uppercase text-white/50 tracking-widest mb-2">Reporting Leaders</p><p className="text-6xl font-serif font-black">{teamStats.count}</p></div>
                        <div className="bg-white p-10 rounded-[40px] shadow-sm border"><h3 className="text-xs font-black uppercase text-slate-400 tracking-widest mb-10">Pillar Averages</h3><PillarBar label="Bedrock" value={teamStats.bAvg} color="#002147" /><PillarBar label="Fuel" value={teamStats.fAvg} color="#C5A059" /><PillarBar label="Purpose" value={teamStats.pAvg} color="#64748b" /></div>
                    </div>
                    <div className="lg:col-span-8 bg-white rounded-[40px] shadow-sm border overflow-hidden"><div className="p-8 border-b bg-slate-50 flex justify-between items-center"><h3 className="text-xs font-black uppercase text-slate-400 tracking-widest">Individual Team Breakdown</h3><div className="bg-white px-4 py-1.5 rounded-full border text-[10px] font-black text-[#C5A059] shadow-sm">LIVE FEED</div></div>
                        <div className="overflow-x-auto"><table className="w-full text-left"><tbody className="divide-y">{teamStats.members.map(m => (<tr key={m.id} className="hover:bg-slate-50/50 transition-all group"><td className="p-8"><div className="flex items-center gap-4"><div className="h-10 w-10 bg-slate-100 rounded-full flex items-center justify-center font-black text-[#002147] group-hover:bg-[#002147] group-hover:text-white transition-all">{m.userName.charAt(0)}</div><p className="font-bold text-lg">{m.userName}</p></div></td><td className="p-8"><span className="px-3 py-1 rounded text-[10px] font-black uppercase border" style={{color: ARCHETYPES[m.archetype]?.color, borderColor: ARCHETYPES[m.archetype]?.color}}>{m.archetype}</span></td><td className="p-8"><div className="flex gap-4"><div className="w-16"><PillarBar label="B" value={m.scores.b} color="#002147" small /></div><div className="w-16"><PillarBar label="F" value={m.scores.f} color="#C5A059" small /></div><div className="w-16"><PillarBar label="P" value={m.scores.p} color="#64748b" small /></div></div></td></tr>))}</tbody></table></div>
                    </div>
                </div>
            )}
        </div>
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-10 text-[#002147]">
        <div className="max-w-7xl mx-auto"><div className="flex justify-between items-center mb-10"><div className="flex items-center gap-4"><BarChart3 className="text-[#C5A059]" size={24} /><h1 className="text-2xl font-serif font-bold">Campus Admin</h1></div><button onClick={()=>setView('welcome')} className="font-bold text-[#C5A059] flex items-center gap-2 bg-white px-4 py-2 rounded-lg border shadow-sm"><ArrowLeft size={16}/> Exit</button></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
              ...
              <div className="bg-white p-8 rounded-3xl shadow-sm border text-emerald-600 font-bold flex items-center gap-2">{db ? <CheckCircle2 size={24}/> : <RefreshCw className="animate-spin"/>} {db ? "Database Live" : "Connecting..."}</div>
            </div>
            <div className="bg-white rounded-[32px] shadow-sm border overflow-hidden overflow-x-auto"><table className="w-full text-left"><thead className="bg-slate-50 border-b"><tr><th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Leader</th><th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Archetype</th><th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Scores</th><th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400">Date</th></tr></thead><tbody className="divide-y divide-slate-50">
                    {assessments.length === 0 ? <tr><td colSpan="4" className="p-20 text-center text-slate-400 italic font-serif">Waiting for cloud data...</td></tr> : assessments.map(e => (<tr key={e.id} className="hover:bg-slate-50/50"><td className="p-6 font-bold">{e.userName}</td><td className="p-6"><span className="px-2 py-1 rounded text-[10px] font-black uppercase border" style={{color: ARCHETYPES[e.archetype]?.color, borderColor: ARCHETYPES[e.archetype]?.color}}>{e.archetype}</span></td><td className="p-6 font-mono text-xs">{e.scores?.b}% / {e.scores?.f}% / {e.scores?.p}%</td><td className="p-6 text-slate-400 text-sm">{e.timestamp ? new Date(e.timestamp).toLocaleDateString() : 'N/A'}</td></tr>))}
                </tbody></table></div>
        </div>
    </div>
  );

  return <div className="p-20 text-center">Loading Worthy X-Ray...</div>;
}
